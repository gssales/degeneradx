...
const options = {
    ...
    const target = args.shift()
		...
		if (target && target.match(/^#([\w\d-]+)$/i)) {
			const channel = message.guild.channels.cache.find((c) => c.name === target.slice(1))
console.log(channel);
			if (
				channel &&
				!channel
	...
	leave: (message, _) => {
		if (message.channel.parentID) {
			...
			message.reply('Você não pode sair deste canal').then((m) => m.react('❌'))
		}
	},
}
